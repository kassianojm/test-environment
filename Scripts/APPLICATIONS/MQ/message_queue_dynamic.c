  	...
  	if (controll == 0)
   	{
		...
		if (flagState == 1 && (second != last_second)) {
				...
				//get maximum throughput ever 	
				if (global_avg_spark >= maxth){
					maxth = global_avg_spark;
					stop = 0;
				} 
				//get some performance loss
				if ((1-(global_avg_spark)/maxth )*100 > 0 ) 
				{
				//	if (second != last_second) 
				//	{
						measure = ((1-(global_avg_spark)/maxth)*100);
						last_second++;
				//	}
				}
			...
			*/
			...
			if ( (cDELAY > window && measure >= loss )  || state > qosmax || (state > qosbase && qosbase > qosmin))
			{
                                vector[0] = 1000;
				qosbase = state - 1;
			...
			//	if (qosbase < qosmin) qosbase = qosmin;
				...
				if ( stop == 0 && state >= qosbase) {
					if (state > qosmin ){
						qosbase = state +1;	
					}else if ( state <= qosmin ){
						qosbase++;
					}
				}else if (state >= qosbase){
					qosbase = state -1;
					stop =1;
				}
				vector[0] = 0;
			...
			//does change qos value before computing realy start	
			if (cTIMEP == 0){
				qosbase = qosmin;
				vector[0] = 0;
