...
import org.yggdrasil.receivers.SparkClusterMPIAInrReceiver;
import org.yggdrasil.receivers.SparkClusterMPIByteArrayReceiver;
import org.yggdrasil.receivers.SparkClusterMPIInrReceiver;
import org.apache.spark.streaming.api.java.JavaMapWithStateDStream;


public class AIntGlobalSUMServer extends AbstractClusterServer implements Serializable{
    ...
    public static void main(String[] args) throws Exception {(new AIntGlobalSUMServer()).run(args);}
    ...
    protected void register_spark_application() throws IOException {
        ...
        for (int i=0; i<senders.length; i++){
            for(int j = 0;j < receivers_by_sender; j++) {
                ...
                JavaReceiverInputDStream<Tuple2<Tuple2<Integer,Integer>, float[]>>  lines = ssc.receiverStream(new SparkClusterMPIAInrReceiver(in_port, sender));
                in_port+=2;
                JavaPairDStream<Tuple2<Integer, Integer>, Tuple2<Integer,Double>> sums = lines.mapToPair(new PairFunction<Tuple2<Tuple2<Integer,Integer>,float[]>, Tuple2<Integer,Integer>, Tuple2<Integer,Double>>() {

                    @Override
                    public Tuple2<Tuple2<Integer, Integer>, Tuple2<Integer, Double>> call(
                            Tuple2<Tuple2<Integer, Integer>, float[]> t) throws Exception {
                        // TODO Auto-generated method stub
                        float[] x = t._2();
                        double sums = 0;
                        for(int i=0; i<x.length; i++) {
                            sums += x[i];
                        } return new Tuple2<>(t._1, new Tuple2<>(x.length, sums));
                    }
                }).reduceByKey((gauche, droite) ->
                                    new Tuple2<Integer, Double>(
                                        gauche._1+droite._1, gauche._2 + droite._2));
        ...
        JavaMapWithStateDStream<Tuple2<Integer,Integer>, Tuple2<Integer, Double>, Tuple2<Integer,Double>, Tuple2<Tuple2<Integer,Integer>, Tuple2<Integer, Double>>> mapWithStateDStream =
                ...
                }).mapWithState(StateSpec.function(mappingFunction).partitioner(partitioner));
