...
public class IntGlobalSUMServer extends AbstractClusterServer implements Serializable{
    ...
    public static void main(String[] args) throws Exception {(new IntGlobalSUMServer()).run(args);}
    ...
    protected void register_spark_application() throws IOException {
        ...
        for (int i=0; i<senders.length; i++){
            for(int j = 0;j < receivers_by_sender; j++) {
                ...
                JavaReceiverInputDStream<Tuple2<Tuple2<Integer,Integer>, Tuple2<Integer, Double>>>  lines = ssc.receiverStream(new SparkClusterMPIInrReceiver(in_port, sender));
                in_port+=2;
                JavaPairDStream<Tuple2<Integer, Integer>, Tuple2<Integer,Double>> sums = lines.mapToPair(x -> x).
                            reduceByKey(
                                    (gauche, droite) ->
